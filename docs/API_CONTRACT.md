...
- `CoachSuggestion` includes optional `routine_action` values (for example, `daily-checkin` or `weekly-review`) so the UI can trigger routines directly from Coach Mode suggestions, plus optional `action`/`target` fields for non-routine flows (indexing, settings, health, or query re-runs) and an optional `project` hint for scoping.
