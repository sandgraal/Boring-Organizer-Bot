...
2. **Coach Mode UI integration** – The UI exposes a toggle and suggestions list, and coach suggestions now pull from health signals (feedback gaps, staleness, low coverage) with routine actions or quick actions (open indexing/health/settings, run query) when available; deeper Fix Queue remediation and connector toggles remain planned.
