...
- `GET /health/fix-queue` – returns failure signals (not-found frequency, metadata gaps, low indexed volume, low retrieval hit rate, repeated questions, permission denials) and prioritized Fix Queue tasks derived from feedback, metadata deficits, denied write attempts, and capture lint issues.
