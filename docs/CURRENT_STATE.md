...
6. **Decision management** – `bob extract-decisions`, `bob decisions` (including `--older-than` cadence filters), `bob decision`, `bob decision-history`, and `bob supersede` wrap the NLP-based extractor (`bob.extract.decisions`) plus the decision table stored with metadata.
