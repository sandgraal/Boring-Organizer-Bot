...
- `GET /health/fix-queue` – returns failure signals (not-found frequency, metadata gaps, repeated questions, permission denials) and prioritized Fix Queue tasks derived from feedback, metadata deficits, and denied write attempts.
...
4. **Health dashboard follow-through** - A Fix Queue dashboard and ingestion telemetry are described in the implementation plan; `/health/fix-queue` now delivers the failure signals (not-found frequency, metadata gaps, repeated questions, permission denials), but task remediation beyond running routines remains manual.
