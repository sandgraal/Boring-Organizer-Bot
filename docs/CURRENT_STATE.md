...
9. **MCP Server** – `bob mcp` starts a local-only JSON-RPC MCP server exposing grounded `ask`, project stats, index status, and permissioned vault read/write (with dry-run).
...
- `GET /decisions` – list extracted decisions with optional filters (`project`, `status`, `older_than_days`).
...
- The Ask page features a polished welcome experience with: an inviting "What would you like to know?" heading, feature hints (ask, click sources, 100% local), dynamic stats showing indexed documents/projects, and keyboard shortcuts (/ to focus, Enter to search).
...
1. **Routines & Fix Queue depth** - The detailed workflow in `docs/ROUTINES_SPEC.md` guides the captures and citations. `/routines/daily-checkin`, `/routines/daily-debrief`, `/routines/weekly-review`, `/routines/meeting-prep`, `/routines/meeting-debrief`, `/routines/new-decision`, `/routines/trip-debrief`, and `/routines/trip-plan` now write their canonical templates into `vault/routines/`, `vault/meetings/`, `vault/decisions/`, and `vault/trips/` with cited retrieval buckets, and `POST /feedback` feeds `GET /health/fix-queue`. Capture lint tasks now surface in Fix Queue, and the Health UI offers one-click open for lint/metadata fixes, while calendar connectors remain planned.
2. **Coach Mode UI integration** – The UI exposes a toggle and suggestions list, and coach suggestions now pull from health signals (feedback gaps, staleness, low coverage) with routine actions or quick actions (open indexing/health/settings, run query) when available; deeper Fix Queue remediation remains planned.
